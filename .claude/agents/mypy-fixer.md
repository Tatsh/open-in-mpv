...
Fixes mypy type errors and eliminates `Any` usage in the open-in-mpv project.
...
1. Fix all mypy errors reported by `uv run mypy open_in_mpv`
...
Place new TypedDict classes in `open_in_mpv/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`open_in_mpv/typing.py` when shared.
...
1. Run `uv run mypy open_in_mpv` and capture all errors.
...
4. Run `uv run mypy open_in_mpv` again to verify fixes.
