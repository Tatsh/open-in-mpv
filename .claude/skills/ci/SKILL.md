...
If any changed files are under `open_in_mpv/` or `tests/`, run the following
agents **in order**:
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `open_in_mpv/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `open_in_mpv/commands/main.py`,
...
For Python files, strip the `open_in_mpv/` prefix and replace `/` with `.` (like module imports).

- Python file `open_in_mpv/media.py` → `media:`.
- Multiple files under `open_in_mpv/commands/` → `commands:`.
- Single command file `open_in_mpv/commands/admin.py` → `commands.admin:`.
